...
import { requestProviderCancel, requestProviderRefill } from "@/lib/providers";
...
export async function resolveOrderRequestAction(id: string, decision: string, note = ""): Promise<RequestState> {
  ...
  if (forward) {
    ...
    if (outcome.error) {
      ...
      await logActivity(admin.id, "admin.request.forward.failed", `${request.type} #${request.publicId}: ${outcome.error}`);
    }
  }
