import { sellPrice } from "./provider-catalogue";
...
/**
 * Reads a provider's catalogue and applies it to what is already stocked.
 *
 * It never takes on anything new. A provider adding two thousand services
 * overnight should not quietly become two thousand rows nobody asked for, so
 * choosing what to stock is a separate, deliberate act: /admin/providers/import,
 * where an operator picks the category and ticks the services themselves.
 */
export async function syncProviderCatalogue(provider: Provider): Promise<SyncReport> {
